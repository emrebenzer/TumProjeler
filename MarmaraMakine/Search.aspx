<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Search.aspx.cs" Inherits="MarmaraMakine.Search" %>
...
<%if (dil == "EN")
   ...
   <img src='resim/tr.png' width='13' height='10' />&nbsp;&nbsp;<a href='Search.aspx?lang=TR&&kelime=<%=Request.QueryString["kelime"] %>'>TÜRKÇE</a>
   <%}
     else
     { %>
   <img src='resim/en.png' width='13' height='10' />&nbsp;&nbsp;<a href='Search.aspx?lang=EN&&kelime=<%=Request.QueryString["kelime"] %>'>ENGLISH</a>
   <%} %></div>
</div>
...
<div id="icerik-baslik">
  ...
  <%if (dil == "EN")
    { %>
<span style="font-size:17px;font-weight:bold;font-family:Arial, Helvetica, sans-serif">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<asp:Label Text="Search Results"  runat="server" /></span><br /><br />
<%}
    else
    { %>
    <span style="font-size:17px;font-weight:bold;font-family:Arial, Helvetica, sans-serif">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<asp:Label ID="Label1" Text="Arama Sonuçları"  runat="server" /></span><br /><br />
<%} %>
      <asp:Repeater runat="server" ID="rptAramaSonuc">
          <ItemTemplate>
         <%if (dil == "EN")
           {%>
               <div class="arama"><a href="Details.aspx?lang=EN&&id=<%# Eval("MakineID") %>"><%# Eval("MakineIsimEN").ToString().Replace(kelime, "<b><i>" + kelime + "</i></b>")%></a><br />
      <span><%# (Eval("MakineIcerikEN").ToString().Length > 300) ? Eval("MakineIcerikEN").ToString().Substring(0, 300).Replace("<br />", "").Replace("<br>", "").Replace(kelime, "<b><i>" + kelime + "</i></b>") : Eval("MakineIcerikEN").ToString().Replace("<br />", "").Replace("<br>", "").Replace(kelime, "<b><i>" + kelime + "</i></b>")%>...</span>
      </div>
           <%}
           else
           {%>
           <div class="arama"><a href="Details.aspx?lang=TR&&id=<%# Eval("MakineID") %>"><%# Eval("MakineIsimTR").ToString().Replace(kelime, "<b><i>" + kelime + "</i></b>")%></a><br />
      <span><%# (Eval("MakineIcerikTR").ToString().Length > 300) ? Eval("MakineIcerikTR").ToString().Substring(0, 300).Replace("<br />", "").Replace("<br>", "").Replace(kelime, "<b><i>" + kelime + "</i></b>") : Eval("MakineIcerikTR").ToString().Replace("<br />", "").Replace("<br>", "").Replace(kelime, "<b><i>" + kelime + "</i></b>")%>...</span>
      </div>
           <%} %>
      ...
      <asp:Label Text="" runat="server" ID="lblBos"/>
